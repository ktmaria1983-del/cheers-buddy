...
// Load user + role
  ...
  useEffect(() => {
  setLoading(true); // prevent flicker

  // REAL-time session listener (this is the correct one)
  ...
  // Load the first session (but do NOT trust it alone)
  ...
  if (!user) {
    supabase.auth.getSession().then(({ data }) => {
      if (data.session) {
        // Force client to forget cached session
