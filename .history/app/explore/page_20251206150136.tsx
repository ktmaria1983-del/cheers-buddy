  ...
  const channel = supabase
    ...
    .on(
      ...
      (payload) => {
        ...
        // Update only the changed win
        setWins((prev) =>
          prev.map((w) =>
            w.id === updated.id
              ? { ...w, cheer_count: updated.cheer_count }
              : w
          )
        );
      }
...
if (data?.id) {
  ...
  // --- FIX: Accurate position with scroll ---
  ...
  for (let i = 0; i < 8; i++) {
    ...
    // Perfect centre of button
    s.style.left = `${startX}px`;
    s.style.top = `${startY}px`;

    // Random directions
    s.style.setProperty("--x", `${Math.random() * 60 - 30}px`);
    s.style.setProperty("--y", `${Math.random() * -80}px`);
...
// Make sure cheer_count is always a number
const newCount = updated?.cheer_count ?? null;
...
setWins((prev) =>
  prev.map((w) =>
    w.id === id
      ? {
          ...w,
          cheer_count: newCount !== null ? newCount : w.cheer_count + 1
        }
      : w
  )
);
