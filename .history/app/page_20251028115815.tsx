...
const fmtUTC = (iso: string) => new Date(iso).toISOString().replace("T", " ").slice(0, 16) + " UTC";
...
function Navbar() {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-40 backdrop-blur bg-slate-900/60 border-b border-white/10">
      <div className="mx-auto max-w-5xl flex items-center justify-between px-4 py-3">
        <div className="flex items-center gap-2">
          <span className="text-2xl">🎉</span>
          <Link href="/" className="text-lg font-bold tracking-tight text-sky-400 hover:text-sky-300">
            Cheers Buddy
          </Link>
        </div>
            ...
            return (
              <Link
                ...
                className={`text-sm font-medium transition ${
                  active ? "text-sky-400 border-b-2 border-sky-400 pb-[2px]" : "text-slate-300 hover:text-sky-300"
                }`}
              >
                {l.label}
              </Link>
